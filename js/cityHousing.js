...
var CityHousing = function(target, fileName, color, name) {
	var self = this;
	var sizeModifier = 1;
	...
	self.generate = function() {
		...
		self.y = d3.scaleLinear().domain([0, 1800000])
		...
		**********/
		...
		var line = d3.line()
			...
			.y(function(d) {
				return self.height - self.y(d.price);
			});
		...
		var areaUnder = d3.area()
    		...
    		.y1(function(d) { return self.height - self.y(d.price); });
	    ...
	    self.lastValue = self.dataset[self.dataset.length - 1].price;
			self.lastCount = self.dataset[self.dataset.length - 1].count;

	    /** for start point */
	    self.firstDate = self.dataset[0].date;
	    self.firstValue = self.dataset[0].price;

	    drawFocus();
	    drawAnnotations();

        self.displayText = self.canvas.append("text")
            .classed("unselectable", true)
            .attr("font-size", "30px")
            .attr("transform", "translate(" + self.width / 2 + "," + 120 + ")")
            .attr("font-family", "Open Sans")
            .attr("text-anchor", "middle")
            .attr("fill", color)
            .style("opacity", 0);
     ...
     self.display = function(percent) {
         self.displayText
            .text(percent)
            .transition("fadeDisplay")
            .duration(500)
            .style("opacity", 1);
     }
	...
	function drawAnnotations() {
		...
		var annotations = [{
		  note: {
		  	title: d3.format("$,d")(self.lastValue),
		  ...
		  subject: { radius: 5, radiusPadding: 5 }
		  	...
		  	title: d3.format("$,d")(self.firstValue),
		...
		if (typeof d0 != 'undefined' && typeof d1 != 'undefined') {
			...
			if (prevPoint != point) {
				...
				self.focus.select(".focusCircle")
					...
					.attr("cy", self.height - self.y(point.price));
				...
				self.focus.select("#part")
					...
					.attr("y2", self.height - self.y(point.price) + self.lineOffset);


				var midY = (self.height - self.y(point.price) - self.lineOffset
							+ graphOffsetTop + self.lineOffset) / 1.3;
